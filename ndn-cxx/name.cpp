...
BOOST_CONCEPT_ASSERT(
    (boost::RandomAccessIterator<Name::const_reverse_iterator>));
...
Name::Name() : m_wire(tlv::Name) {}

Name::Name(const Block& wire) : m_wire(wire) { m_wire.parse(); }

Name::Name(const char* uri) : Name(std::string(uri)) {}
...
Name::Name(std::string uri) {
  ...
  while(iComponentStart < uri.size()) {
    ...
    if(iComponentEnd == std::string::npos) iComponentEnd = uri.size();
...
size_t Name::wireEncode(EncodingImpl<TAG>& encoder) const {
...
const Block& Name::wireEncode() const {
...
void Name::wireDecode(const Block& wire) {
  if(wire.type() != tlv::Name) NDN_THROW(tlv::Error("Name", wire.type()));

  m_wire = wire;
  m_wire.parse();
}

Name Name::deepCopy() const {
...
const name::Component& Name::at(ssize_t i) const {
  if(i < 0) {
...
PartialName Name::getSubName(ssize_t iStartComponent,
                             size_t nComponents) const {
  PartialName result;

  if(iStartComponent < 0) iStartComponent += static_cast<ssize_t>(size());
  size_t iStart =
      iStartComponent < 0 ? 0 : static_cast<size_t>(iStartComponent);

  size_t iEnd = size();
  if(nComponents != npos) iEnd = std::min(size(), iStart + nComponents);

  for(size_t i = iStart; i < iEnd; ++i) result.append(at(i));
...
Name& Name::set(ssize_t i, const Component& component) {
  if(i < 0) {
    i += static_cast<ssize_t>(size());
  }
...
Name& Name::set(ssize_t i, Component&& component) {
  if(i < 0) {
    i += static_cast<ssize_t>(size());
  }

  const_cast<Block::element_container&>(m_wire.elements())[i] =
      std::move(component);
  m_wire.resetWire();
  return *this;
}

Name& Name::appendVersion(optional<uint64_t> version) {
  return append(Component::fromVersion(version.value_or(
      time::toUnixTimestamp(time::system_clock::now()).count())));
}

Name& Name::appendTimestamp(optional<time::system_clock::TimePoint> timestamp) {
  return append(
      Component::fromTimestamp(timestamp.value_or(time::system_clock::now())));
}

Name& Name::append(const PartialName& name) {
  if(&name == this)
    // Copying from this name, so need to make a copy first.
    return append(PartialName(name));

  for(size_t i = 0; i < name.size(); ++i) append(name.at(i));
...
Name& Name::appendParametersSha256DigestPlaceholder() {
...
void Name::clear() { m_wire = Block(tlv::Name); }

// ---- algorithms ----

Name Name::getSuccessor() const {
  if(empty()) {
    static const Name n(
        "/sha256digest="
...
int Name::compare(size_t pos1, size_t count1, const Name& other, size_t pos2,
                  size_t count2) const {
...
void Name::toUri(std::ostream& os, name::UriFormat format) const {
  if(empty()) {
    os << "/";
    return;
  }
...
std::string Name::toUri(name::UriFormat format) const {
...
size_t hash<ndn::Name>::operator()(const ndn::Name& name) const {

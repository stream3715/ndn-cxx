 ...
 * Lesser General Public License along with ndn-cxx, e.g., in COPYING.md file.
...
class Interest : public PacketBase,
 ...
 public:
   ...
   *  @warning In certain contexts that use `Interest::shared_from_this()`,
   * Interest must be created using `make_shared`. Otherwise,
   * `shared_from_this()` will trigger undefined behavior.
   */
  ...
  /** @brief Prepend wire encoding to @p encoder according to NDN Packet Format
   * v0.3.
   */
   ...
   *
   *  The string always starts with `getName().toUri()`. After the name, if any
   * of the Interest's CanBePrefix, MustBeFresh, Nonce, InterestLifetime, or
   * HopLimit fields are present, their textual representation is appended as a
   * query string. Example: "/test/name?MustBeFresh&Nonce=123456"
   */
 ...
 public:  // matching
   ...
   * MustBeFresh processing is limited to rejecting Data with zero/omitted
   * FreshnessPeriod.
   */
   ...
   *
   *  Two Interests match if both have the same Name, CanBePrefix, and
   * MustBeFresh.
   */
 ...
 public:  // element access
   ...
   *
   *  As part of transitioning to NDN Packet Format v0.3, the default setting
   * for CanBePrefix will be changed from "true" to "false". Application
   * developers are advised to review all Interests expressed by their
   * application and decide what CanBePrefix setting is appropriate for each
   * Interest, to avoid breaking changes when the transition occurs. Application
   * may either set CanBePrefix on a per-Interest basis, or declare a default
   * CanBePrefix setting for all Interests expressed by the application using
   * this function. If an application neither declares a default nor sets
   * CanBePrefix on every Interest, Interest::wireEncode will print a one-time
   * warning message.
   *
   *  @note This function should not be used in libraries or in ndn-cxx unit
   * tests.
   ...
   *
   *  If the Nonce element is present, the new nonce value will differ from the
   * old value. If the Nonce element is not present, this method does nothing.
   */
  void refreshNonce();
   ...
   * ApplicationParameters element. Else, if the block's TLV-TYPE is
   * ApplicationParameters, it will be used directly as this Interest's
   * ApplicationParameters element. Else, the block will be nested into an
   *  ApplicationParameters element.
   *
   *  This function will also recompute the value of the
   * ParametersSha256DigestComponent in the Interest's name. If the name does
   * not contain a ParametersSha256DigestComponent, one will be appended to it.
   */
  Interest& setApplicationParameters(const Block& parameters);

  /** @brief Set ApplicationParameters by copying from a raw buffer.
   *  @param value points to a buffer from which the TLV-VALUE of the parameters
   * will be copied; may be nullptr if @p length is zero
   ...
   *  @param value buffer containing the TLV-VALUE of the parameters; must not
   * be nullptr
   *  @return a reference to this Interest
   *
   *  This function will also recompute the value of the
   ...
   *
   *  This function will also remove any ParametersSha256DigestComponents from
   * the Interest's name.
   */
  ...
  Interest& setAgentNodeID(Name agentid) {
    m_agentid = agentid;
    m_wire.reset();
    return *this;
  }
 ...
 public:  // ParametersSha256DigestComponent support
   ...
   *
   *  Returns true if there is a single ParametersSha256DigestComponent in the
   * name and the digest value is correct, or if there is no
   * ParametersSha256DigestComponent in the name and the Interest does not
   * contain any parameters. Returns false otherwise.
   */
 ...
 private:
   ...
   *
   *  @pre The name is assumed to be valid, i.e., it must not contain more than
   * one ParametersSha256DigestComponent.
   ...
   *
   *  @retval pos The name contains exactly one ParametersSha256DigestComponent
   * at index `pos`.
   *  @retval -1  The name contains zero ParametersSha256DigestComponents.
   *  @retval -2  The name contains more than one
   * ParametersSha256DigestComponents.
   */
 ...
 private:
  ...
  // Stores the "Interest parameters", i.e., all maybe-unrecognized non-critical
  // TLV elements that appear at the end of the Interest, starting from
  // ApplicationParameters. If the Interest does not contain any
  // ApplicationParameters TLV, this vector will be empty. Conversely, if this
  // vector is not empty, the first element will always be an
  // ApplicationParameters block. All blocks in this vector are covered by the
